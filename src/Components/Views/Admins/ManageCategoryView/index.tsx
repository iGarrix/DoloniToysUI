...
import {
	GetAllCategory,
	RemoveCategory,
} from '../../../../Redux/reducers/categoryReducer/action';
		...
		<div className={`${style.container}`}>
			<div className={`${style.infoContainer}`}>
				<h1 className={`${style.message}`}>
					{t('There are')} {categories?.total ? categories.total : 'none'}{' '}
					{t('pages and')} {categories ? categories.totalObj : 'none'}{' '}
					{t('categories,')}{' '}
					<span
			...
			<div
				className={`${style.paginatorContainer} ${
					categories?.total === 1 ? style.hide : style.visible
				}`}>
				<Paginator
					total={categories?.total ? categories?.total : 0}
