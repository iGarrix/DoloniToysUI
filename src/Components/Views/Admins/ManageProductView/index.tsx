		...
		<div className={`${style.container}`}>
			<div className={`${style.infoContainer}`}>
				<h1 className={`${style.message}`}>
					{t('There are')} {products?.total ? products.total : 'none'}{' '}
					{t('pages and')} {products ? products.totalObj : 'none'}{' '}
					{t('products,')}{' '}
					<span
						...
						{products?.pageables?.map((item, index) => {
							return (
								<tr key={index} className={`${style.block}`}>
									<td
										className={`${style.item} w-[45%] cursor-pointer`}
										onClick={() => {
									...
									<td
										...
										{moment(item.create).format('dddd DD.MM.YYYY HH:mm')}
									</td>
									<td className={`${style.item} w-[10%]`}>
										<button
									...
									<td className={`${style.item} w-[10%]`}>
										<button
											...
											onClick={() => {
												dispatch(productSlice.actions.initSelectProduct(item));
												nav(
													'edit-product/' + ReplaceArticle(item.article, true)
												);
											}}>
											{t('Edit')}
										</button>
			...
			<div
				className={`${style.paginatorContainer} ${
					products?.total === 1 ? style.hide : style.visible
				}`}>
				<Paginator
					total={products?.total ? products?.total : 0}
					onPaginate={onPaginate}
					inititalPage={initPage}
				/>
			</div>
		</div>
	);
